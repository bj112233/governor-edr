...
### [2026-07-24] Packaging migration mixed tool-swap with version upgrades, breaking test isolation
- **Mistake**: First `uv lock` run resolved to latest-compatible versions (yarl 1.23→1.24, yfinance 1.3→1.5, zopfli, etc.) because the legacy `requirements.txt` had hand-pinned `==X.Y.Z` exact versions while the new `pyproject.toml` used wide `>=` bounds. The packaging-migration commit silently became a 30+ package upgrade commit, causing 1 test failure and a 0.75% coverage drop that were impossible to attribute to either the tool-swap or a specific package bump.
- **Root cause**: Conflated two independent changes — (1) replacing the dependency management tool (requirements.txt → pyproject.toml + uv.lock) and (2) upgrading package versions. The resolver did exactly what it was told: wide bounds + no constraints = latest. There was no mechanism to carry the old exact pins forward as resolver constraints.
- **Rule**: When migrating dependency manifests (e.g. requirements.txt → pyproject.toml), the migration commit MUST reproduce the previous venv bit-for-bit. Use `[tool.uv] constraint-dependencies = ["pkg==old_version", ...]` (or `uv lock --constraint old-pins.txt` where supported) to pin the resolver to the legacy exact versions for the migration commit only. Remove the constraints in a SEPARATE `uv lock --upgrade` commit so version bumps are isolated and bisectable. If multiple tests fail on upgrade, use `uv lock --upgrade-package <name>` per-package rather than a blanket upgrade. Never lower a coverage ratchet baseline due to version-drift noise — only lower it after isolating the upgrade and confirming the drop is legitimate (e.g. code path became unreachable in the new version).

### [2026-07-25] 99% unit coverage on process_analyzer hid dead wiring to kill-queue
- **Mistake**: Built `analyze_process_event` (Sysmon-enriched wrapper with 4 new TTP checks: T1059.005, T1027, T1548.002, T1036) with 99% line coverage and 106 tests. Declared the module "done" and moved on. But `monitor_analyzer._diff_suspicious_procs:280` still called `analyze_cmdline` (the old function), not `analyze_process_event`. The 4 new checks never ran in production. The kill-queue path (`queue_kill_for_ttp`) was only reachable through the old function. Additionally, `register_malicious_hash` (the feed point for T1027) was never called from anywhere outside `process_analyzer.py` itself — the known-bad hash set was permanently empty.
- **Root cause**: All tests called `analyze_process_event` and `register_malicious_hash` directly from the test file. Every line ran, coverage was 99%, but no test verified that PRODUCTION code calls these functions. Unit coverage measures "did this line execute in some test" not "does the production wiring actually invoke this module". The integration gap was invisible because the tests were structurally identical to how the module would be called — but the actual caller was never updated.
- **Rule**: Every new module that feeds an existing pipeline (kill-queue, alert dispatcher, threat hunter) MUST have an E2E integration test: input → new module → REAL existing consumer → mock only the terminal side effect (e.g. `queue_kill_for_ttp`). Unit coverage on the module in isolation is necessary but NOT sufficient. Added as AGENTS.md §6. The integration test (`test_monitor_analyzer_sysmon_integration.py`) would have caught both bugs on day one: it imports `SnapshotDiffer` (the real consumer) and asserts `queue_kill_for_ttp` was called. A separate lesson: when a module has a "populated at runtime" comment (`# populated at runtime by threat feed sync`), grep for the populator — if nothing calls it, the module is a no-op regardless of test coverage.
